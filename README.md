...
同じ数で､有効な members ミラーがあれば全件読みを省略し､次の interval まで待つ｡
diff が必要なときはフォロー全件を読み､members は 30 日未満のローカルミラーを使う｡
ミラーが使えなければ members も全件読む｡フォローは Owned Reads､members は Users として
...
前回成功した diff のフォロー数と probe の結果が同じで､有効な members ミラーがあれば､
...
members のミラーは `$XDG_STATE_HOME/twigpui/sync_members.json` に保存する｡
全件取得から 30 日未満なら再利用し､追加・削除が成功するたびに更新する｡
無い・壊れた・別 list 向け・期限切れ・未来の取得時刻のミラーは全件取得で置き換える｡
この取得を先に保存するので､続くフォロー取得が失敗しても members を買い直す必要はない｡
x.com での手編集は最大 30 日見えない｡今すぐ members を読み直すにはこのファイルを削除して
`--sync-list --reread` を実行する (`--reread` 単独では有効なミラーを再利用する)｡
debug ビルドの保存先は `twigpui-dev`｡再取得時には古いミラーと新しい一覧の差分件数をログに残す｡
