...
- **先頭にいるとき** — そのまま流れ込み､滑るように視界へ降りてくる｡開きっぱなし
  ...
  切り替える; ホイールに触れた瞬間に滑りは止まり､toast を押せば続きから
  再開する｡
- **下の方を読んでいるとき** (または follow が off のとき) — リストの下端に
  ...
  先頭にいるときと同じように滑り込んで表示される｡toast はスクロールしても
  下端に留まり､流れ込みの間はまだ視界の上にある件数を数え下げて､0 で消える
  (#206)｡
...
`X_LIST_ID` を設定して `X_AUTO_SYNC_LIST=true` にすると､ウィンドウは開いている間ずっと､
そのリストのメンバーをフォロー中のアカウントに追従させつづける｡フォローしたアカウントは追加され､
フォローを外したアカウントは削除される｡**既定では無効** — タイマーで課金するので､頼まれるまで回さない｡
無効の間も､下のステータスバーからの手動同期と `--sync-list` は使える｡

**アプリが触るのは､自分がリストに入れたアカウントだけ｡** 同期は最初に一度だけリストのメンバーを
全件読み､それ以降は自分の追加と削除を `sync_members.json` に記録して､そこから diff を取る｡
だからその後に x.com で手で追加したアカウントは同期から見えず､削除もされない｡手で外したアカウントは
記録に残るので､足し直されない｡最初の全件読みの時点でリストにいたアカウントは記録に入るので､
フォローしていなければ削除の対象になる｡
...
ウィンドウが 520px より狭いと文言を詰める ("Sync in 5h 12m"､"in 4m"､"posts")｡
それでも footer に入らなければ次の sync の文言を "…" で切り､post の数は
右端に残る｡
...
`X_AUTO_SYNC_LIST` が既定の `false` のままならタイマーは回らず､同期はこのボタンでしか起きない｡
その実行はやることが無くなった時点で止まる｡

**タイマーで金を使う｡** 最初にフォロー数を確認する (Owned 1 件)｡前回の成功した diff と
同じ数で､members の記録があれば全件読みを省略し､次の interval まで待つ｡
diff が必要なときはフォロー全件を読み､members はローカルの記録を使う｡
...
debug ビルドも同期するが､フォローグラフを読まず､固定のシード screen name を
development 用のリストへミラーする (#169)｡members のミラーは同じ仕組みを使い､
保存先には独自の `twigpui-dev` state ディレクトリを使う｡
...
```sh
cargo run --release -- --sync-list          # dry run: 未送信の計画とフォロー数を確認し､必要なら diff
cargo run --release -- --sync-list --reread # 計画のガードと count による省略を越えて diff を買い直す
...
**dry run は無料ではない｡** 全件読みの前に､フォロー数の probe (Owned 1 件) を行い､
読み取る見込みの件数と種類を stderr に表示する｡フォロー一覧は Owned Reads､members は Users
(Owned の 10 倍の単価)｡価格は developer console で確認する｡

設定中の list 向けの計画に未送信の追加が残っていれば､dry run は API を読まずにその計画を表示する｡
`--apply` なら読み直さず送信できる｡`--reread` はこのガードを越え､新しい diff で計画を置き換える｡

前回成功した diff のフォロー数と probe の結果が同じで､members の記録があれば､
フォロー一覧と members の読み取りを省略する｡`--reread` で省略を解除できる｡
確認の間に同数の follow と unfollow があると､次に数が動くか強制実行するまで見逃す｡
debug ビルドは固定 seed を同期するため､フォロー数による省略は行わない｡

members の記録は `$XDG_STATE_HOME/twigpui/sync_members.json` に保存する｡リストの写しではなく､
このアプリがリストに入れたアカウントの台帳である｡最初の全件取得を種にし､追加・削除が成功するたびに更新する｡
**古さでは読み直さない｡** members の全件取得はフォロー一覧の 10 倍高く､残高が足りなければ途中で止まって
何も残らないからである｡全件取得が走るのは､ファイルが無い・壊れている・別 list 向けのときだけ｡
この取得を先に保存するので､続くフォロー取得が失敗しても members を買い直す必要はない｡
x.com での手編集は同期から見えない｡リストの実態に合わせ直したいときはこのファイルを削除して
`--sync-list --reread` を実行する (`--reread` 単独では記録を再利用する)｡
debug ビルドの保存先は `twigpui-dev`｡
...
**ここでは**削除に `--prune` が要る｡最初の全件取得の時点でリストにいたアカウントは記録に入るので､
手で追加したものが削除の候補に混ざる可能性があり､それを消すかどうかは利用者の判断のままにする｡上の
バックグラウンド同期は `sync_prune_limit_percent` の範囲で尋ねずに削除する｡どちらの場合も､
記録に無いアカウント (最初の全件取得より後に手で追加したもの) には届かない｡

計画に未送信の追加が残っていて記録がまだ無いときは､送る前に `--sync-list --reread` で記録を作る｡
先に送り切ると､記録の種になる全件取得が､追加した数だけ大きくなったリストに対して走る｡
dry run はこの状況を検知して同じことを言う｡
...
| `⌘N` | コンポーザーのウィンドウを開く (既に開いていれば前面へ) |
| `esc` | コンポーザーのウィンドウを閉じる (下書きは保持する｡コンポーザーの窓では `⌘W` も同じ) |
| `⌘↑` | 最新の投稿へ戻る |
| `⌘Q` | 終了する |
| `⌘W` | ウィンドウを閉じる |
| `⌘M` | 最小化する |
...
| `l` | 選択中の投稿をいいね / 取り消す |
| `r` | 選択中の投稿をリポスト / 取り消す |
| `n` | コンポーザーのウィンドウを開く (`⌘N` と同じ) |

`⌥⌘F` と `⌥⌘T` は常駐させたときのためのもので (#267)､Stickies が Window メニューに置いて
...
**裸のキーは 5 つだけで､どれも key context が守っている｡** #58 が本当に問題に
している危険は､投稿を入力している最中に裸の `j`/`k`/`n` が発火することである｡
#58 の時点ではバインドを 1 つも裸にしないことでそれを避けていた｡#148 が裸の
5 つを入れるにあたって､代わりに述語が避ける — `j`/`k`/`l`/`r`/`n` は
`menu::KEY_CONTEXT` ではなく `menu::BROWSE_CONTEXT` (`Timeline && !Input`) へ
閉じてある｡gpui の `!` はフォーカス経路の**すべての** key context を見るので､
何かテキスト入力にフォーカスがある間はこの 5 つがバインドとして一致しない｡
そのとき打鍵は文字として入力ウィジェットへ流れる｡コンポーザーは #282 で
別ウィンドウへ移り､今の timeline のウィンドウにはテキスト入力そのものが無い
ので実際には効きようがないが､述語は残してある — 将来また何か Input を
足したときに同じ罠を踏まないための門である｡残りのバインドは今までどおり
`⌘` を伴う｡

**選択は index ではなく投稿 ID で持つ｡** タイムラインはリロード・ソースの
切り替え・自動更新の流し込みで並びごと入れ替わる｡index を覚えると読み手の
知らないうちに別の投稿を指すが､ID なら一覧から消えた選択は「選択なし」に
落ちるだけで済み､次の `j` は画面の最上部の行からやり直す｡

**最初の `j` は「進む」ではなく「選ぶ」｡** 選択が無い状態で押したときの起点は
ビューポートの最上部の行である｡先頭へ跳ぶのではない — 下まで読んだところで
`j` を押した人が期待するのは目の前の行だからである｡両端では止まり､巻き戻さない｡

**`l` と `r` はボタンと同じ門を通る｡** 出ていないボタンの操作にキーだけが
届くことはない｡リポストの行では #52 のとおり元の投稿に働く｡
...
**`esc` はコンポーザーのウィンドウを閉じるだけである｡** 下書きは打ったそのまま
`TimelineView` 側に残る (composer 自身は状態を持たない): 誤打で失えば取り返せないし､
下書きを決して失わないことがコンポーザーの主な約束である (#14)｡`⌘N` で開き直せば
続きから書ける｡
...
その横に描く｡1 つの `menu::Shortcut` 定数がキー操作､メニュー用のラベル､**そして
アクション**を持ち (#119)､キーバインドとメニュー項目はすべてそこから組み立てられる
— だから 3 つのうち 1 つだけを変えて､ほかを変えないままにはできない｡#95 で
ヘッダの下のヒントの帯 (もう一つの文言の出し先だった) 自体が消えたので､今は
`menu_label` が唯一の文言である｡
