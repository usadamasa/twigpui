...
`scripts/code-metrics.sh --check` は各ファイルの実装行数を一律の上限
(スクリプトの `max_implementation_lines`､600 行) と突き合わせ､超えていれば
落ちる｡新しいファイルの登録は要らない — 上限が全ファイルに効く｡CI がこれを
実行する｡

上限より大きいまま残っているファイルは `metrics-baseline.tsv` に天井つきで
載っていて､その一覧は縮むことしかできない: 行を足せない､天井を
`origin/main` の値より上げられない､上限を下回ったら行を消す｡落ちたときの
道は分割だけだ｡上限そのものを動かすときは､スクリプトの数字を変えて理由を
pull request に書く — 全ファイルに効く 1 か所の変更なので diff で見落とし
ようがない｡

#48 から #241 まではファイルごとに天井があり､同じ pull request で上げれば
通った (引き上げ 78 回に対して引き下げ 8 回)｡書く人とレビューする人が
同じなら「目に入る」は歯止めにならなかった､というのが #241 の結論だ｡
...
| 添付 4 件 | メディアグリッドの最も広い形と､バッジ (#65)｡2 枚目は縦長 9:16 で､横長の隣で枠を埋める切り取りを見る (#154) |
...
**メディアとアバターは fixture の隣のファイルから読む (#234)｡**
`fixtures/media/` の PNG を fixture からの相対パスで書き､`fixture::load` が
絶対パスにする｡fixture 起動はネットワークへ一切出ないので､オフラインでも
同じ画面になり､本物の取得失敗の WARN と混ざらない｡`https://` で始まる
URL を書けば本物と同じく fetch される｡画像は無地に枠と斜めの帯を描いた
ものなので､切り取られれば枠が欠け､引き伸ばされれば帯が傾く｡
