...
///
/// フェードの最中も含めて､この行が出ている間ずっとこの高さだ｡0 から
/// 補間しないのが要点で､そうすれば timeline が動くのは出現と消失の各 1 回
/// だけになる — 高さも一緒にアニメーションさせると､フェードのフレーム
/// ごとに上の timeline が押し上げられ､読んでいる行が指の下で滑る｡
pub(crate) const SYNC_ROW_HEIGHT: Pixels = px(22.0);

/// 手動 sync の確認ダイアログの幅 (#205)｡
///
/// ウィンドウ幅に対する割合ではなく固定値だ｡中身は 3 つの短い文で､
/// 幅の広いウィンドウでそれを引き伸ばせば 1 行が読み返しにくくなる｡
pub(crate) const SYNC_DIALOG_WIDTH: Pixels = px(360.0);

/// ダイアログの背後を覆う膜 (#205)｡`rgba` なので下 8 bit が不透明度だ｡
///
/// 明暗のどちらのテーマでも同じ黒を使う｡覆いは色ではなく「ここは今
/// 触れない」という合図で､明るいテーマで白く覆うとそれが伝わらない｡
