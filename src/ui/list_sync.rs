...
impl TimelineView {
    ...
    ///
    /// 自分で解決するのではなく､[`Self::start`] がすでに解決した client を
    /// 使い回す｡`oauth::resolve_credential` は refresh token を回して
    /// 書き戻すので､2 つが競合すると保存されたセッションが死にかねない
    /// からだ｡
    ///
    /// #239 以前は､その代償として「手元の access token が非常に長い実行の
    /// 間に古くなること」を受け入れていた — そしてそれがまさに #239 だった｡
    /// 今は client が `oauth::Session` を持ち､期限の手前で自分を更新する｡
    /// 更新は `Mutex` の中で 1 度だけ起きるので､ここが競合を避けるために
    /// 解決を 1 か所に寄せている理由はそのまま残り､代償のほうが無くなった｡
    ///
