...
アクセストークンの失効。`cargo run -- --fetch-only` を一度通せばアプリが更新して保存する。
**probe 側で refresh token を使わない** — 回転するので、アプリの持ち分が無効になる。
理由と課金は `../SKILL.md` の「トークンが失効したら」に書いてある。

失効した 401 には `x-rate-limit-*` も `x-access-level` も付かない。
撃った 2 本はそのまま課金されるので、長い round を始める前に 1 本で生存確認しておくと安い。
