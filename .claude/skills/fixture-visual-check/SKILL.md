...
| 添付 4 件 | 横長が過半数なので縦積みになるメディアと、バッジ (#65, #256)。2 枚目は縦長 9:16 で、積まれても各枚が 1 枚のときと同じ寸法を取ることを見る |
| 画像を持つ引用 | 引用カード自身のメディア (#123) |
| 画像を持つ repost | 外側の行に出る元の post のメディア (#104) |
| 引用の repost | "@user reposted" の行と引用カードが 1 行に同居する (#13)。埋め込まれた post の面をいちばん多く持つ行 |
| 小さい 1 枚 (128×72) | 幅の上限まで拡大され、周りに灰色の枠が無いこと (#256) |
| 縦長 1 枚 | 高さの上限で止まり、本文列の左端から始まること (#256) |
| 返信 | "Show thread" のトグルとそのコスト警告 (#12) |
| 自分の post | Delete を出す唯一の行 (#72) |
...
- `"translucent": true` を書くと fixture のウィンドウが透過で立ち上がる (#267)。
  **`--png` では写らない** (後ろに何も無いため)。透過の確認は `cleanshot-capture` の
  ...
  ウィンドウは手元に無いあいだだけ透けるので、撮る前に別のアプリを前に出しておく。
  Claude のセッションから前に出せるのは `open-artifact ./tmp/probe.png` だけ
