...
- **操作の結果**。クリックもキー入力もしない。状態遷移を見たいなら、
  ...
  #146 の層 3 がそれで、`src/ui/mod.rs` の `#[gpui::test]` 群がその置き場。
  **`window.dispatch_action` がクリックの代わり**になる (`ShowNewPosts` を
  投げて「N new posts」バーを押したのと同じ経路を通す、など)。
...
**この種の不具合は、このリポジトリのどのテストでも捕まえられない。**
gpui はテストプラットフォームでレイアウトを走らせないので、間隔も折り返しも
コードから assert できない。撮って見るのが唯一の手段になる。
